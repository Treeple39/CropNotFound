...
namespace Coffee.UIExtensions
{
...
#endif
    {
        ...
        // 当可见性被关闭时会停止播放。而当恢复显示时,追回之前的显示效果
        public void ContinuePlay()
        {
            if(_startTime == null || _startTime == System.DateTime.MinValue)
            {
                UnityEngine.Debug.LogWarningFormat(this, "[UIParticle] can't revert {0} play.", name);
                return;
            }
            System.TimeSpan delta = System.DateTime.Now - _startTime;
            // 仅相差一帧的情况,就不补了
            if(delta.TotalSeconds < 0.03f)
            {
                UnityEngine.Debug.LogWarningFormat(this, "[UIParticle] tcan't revert{0} play.no interval", name);
                return;
            }
            float deltaSceond = (float)delta.TotalSeconds;
            particles.Exec(p => p.Simulate(deltaSceond, true, true));
            particles.Exec(p => p.Play());
        }
